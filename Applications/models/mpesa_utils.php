...
// $passkey="bfb279f9aa9bdbcf158e97dd71a467cd2e0c893059b10f78e6b72ada1ed2c919";
 ...
 class mpesa_utils extends CI_Model{
 	...
 	function onlineCheckout($phone_number,$amount)
 	{
		...
		$url = 'https://sandbox.safaricom.co.ke/mpesa/stkpush/v1/processrequest';
		 ...
		 // insertcheckID($checkid);
		  ...
		  $sql = "INSERT INTO `mpesa`(`cunkey`) VALUES ('".$checkid."')";

		  $con->query($sql);

		 // $this->db->set('cunkey',$checkid);
	...
	function transactionStatus()
	{
		...
		$sql= "SELECT `cunkey` FROM `mpesa` WHERE `trans_id`=(SELECT max(`trans_id`) FROM `mpesa`)";

		$checkoutrequestid = $con->query($sql);
		...
		$access_token = $this->genAuthToken($this->secret, $this->key);
		 ...
		 //var_dump($response);
		  ...
		  if ($response["ResultCode"]!="0") {
		  	...
		  	//productUpdate($_SESSION['item'],$_SESSION['amt']);
		  }
		//  echo $curl_response;
